...
//            player.moveRight();
        ...
        for (var i = 0; i < blocks.length; i++) {
            ...
            if (this.y > blocks[i].y) {
                ...
                if (blocks[i].monster !== 0 && blocks[i].monster !== undefined) {
                    if (this.x >= blocks[i].x - this.width + 15 && this.x <= blocks[i].x + blocks[i].width - 30 &&
    ...
    this.toggleChoices = function() {
        ...
        switch (currentCol) {
            ...
            case 0:   
                ctx.strokeRect(5, 5, screenWidth/3, screenHeight-5);
                break;
            case 1:   
                ctx.strokeRect(screenWidth/3, 5, screenWidth/3, screenHeight-5);
                break;
            case 2:    ctx.strokeRect(2*(screenWidth/3)-5, 5, screenWidth/3, screenHeight-5);
                break;
        }
    ...
    this.chooseChoice = function() {
        console.log("here");
           switch (currentCol) {
                
            case 0:   
                this.x = screenWidth/3 - 95 ;
                break;
            case 1:   
                this.x = screenWidth/2-40 ;
                break;
            case 2:    
                   this.x = 2*(screenHeight/3)-150;
                break;
        }
    }
